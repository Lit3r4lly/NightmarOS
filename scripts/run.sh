...
qemu-system-"$(./scripts/target-triplet-to-arch.sh "$HOST")" -cdrom "$PROJECT_NAME.iso" -nodefaults -vga std -M q35,smm=off -no-reboot -debugcon file:info.log -d int 2> interrupts.txt